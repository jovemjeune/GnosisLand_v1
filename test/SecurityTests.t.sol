...
contract SecurityTests is Test {
    ...
    function setUp() public {
        ...
        vm.prank(teacher);
        address lessonNFTAddr = factory.createLessonNFT(
            teacherTokenId,
            LESSON_PRICE,
            "Test Course",
            ""
    ...
    function test_RatioManipulation_DepositWithdraw() public {
        ...
        // Should still be 1:1
        assertEq(
            glusd.balanceOf(student),
            deposit + deposit2,
            "Total GlUSD should equal total deposits"
        );
        assertEq(
            treasury.underlyingBalanceOf(student),
            deposit + deposit2,
            "Total underlying should equal total deposits"
        );
    ...
    function test_RatioManipulation_VaultWithdraw() public {
        ...
        assertTrue(
            treasury.underlyingBalanceOf(student) <= deposit,
            "Underlying balance should not exceed original deposit"
        );
